...
class NPC(pg.sprite.Sprite):
    def __init__(self):
        ...
        # self.image.fill(RED)
        self.image_orig = r.choice(meteor_images)# used meteor list not image
...
class Player(pg.sprite.Sprite):
    ...
    def update(self):
        # unhide if hidden
        if self.hidden and pg.time.get_ticks() - self.hide_timer > 5000:
            self.hidden = False
            self.hide_timer = pg.time.get_ticks()
            self.rect.center = (WIDTH / 2, HEIGHT + 500)
...
sounds_folder = path.join(game_folder, "snds")
...
#load in images
background = pg.image.load(path.join(background_folder, "blue.png")).convert()
background = pg.transform.scale(background,(WIDTH, HEIGHT))
background_rect = background.get_rect()
# npc_img = pg.image.load(path.join(enemy_imgs_folder, "rock.png")).convert()
player_img = pg.image.load(path.join(player_imgs_folder, "playerShip1_blue.png")).convert()
lazer_img = pg.image.load(path.join(player_imgs_folder, "laserBlue01.png")).convert()
...
pg.mixer.music.load(path.join(sounds_folder,'tgfcoder-FrozenJam-SeamlessLoop.mp3'))
